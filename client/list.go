...
func (c *Client) List(options ListOptions) ([]record.Record, error) {
	...
	for rows.Next() {
		var r record.Record
		err := rows.Scan(&r.Id, &r.Command, &r.LastUpdate, &r.Count, &r.Tag)
		if err != nil {
...
func buildListQuery(options ListOptions) (string, []interface{}, error) {
	...
	if options.Pattern != "" {
		query.WriteString(fmt.Sprintf(`SELECT h.id, h.command, h.last_update, h.count, t.tag 
		FROM hist h
		  LEFT JOIN tag t ON h.id = t.hist_id AND t.tag = '%s' 
		WHERE regexp('%s', h.command) = true OR t.tag = '%s'`,
			options.Pattern, options.Pattern, options.Pattern))
	} else {
		query.WriteString(`SELECT h.id, h.command, h.last_update, h.count, null 
		FROM hist h`)
	}
	if options.ByCount {
		query.WriteString(" ORDER BY h.count, h.last_update DESC")
	} else {
		query.WriteString(" ORDER BY h.last_update, h.count DESC")
	}
	if options.Limit > 0 {
